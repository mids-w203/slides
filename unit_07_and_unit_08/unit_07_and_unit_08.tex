...
\begin{frame}
  ...
  \note[item]{Here's a specific example to build more intuition.}
  \note[item]{write on slide: 13.2-11.1 = 1.1 outfits}
  \note[item]{Is this a big difference?}
  \note[item]{To be more precise, if there was no difference between our two populations, would a difference of 1.1 outfits be surprising?}
  \begin{exampleblock}{From the Journal of Empirical Fashion}
  \begin{tabular}{c | c | c}
  & New Yorkers & San Franciscans \\
  \hline
black outfits &  12.1 & 13.2  \\
sample size & 50 & 50
 \end{tabular}
 \end{exampleblock}
...
\begin{frame}
  ...
  \note[item]{The top distribution has very high standard deviation. You can see that it doesn't seem unusual to get two data points that are 1.1 outfits apart.}
  \note[item]{The middle distribution has medium deviation. Now it starts to look a little more surprising that our two sample averages were 1.1 apart.}
  \note[item]{The bottom distribution has low standard deviation. Now it seems quite unlikely to get two sample averages 1.1 apart. The data seems inconsistent with this hypothesis.}
    \begin{center}
